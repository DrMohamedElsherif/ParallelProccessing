 ...
 */
 ...
 /////// MY CODE
 __global__ void reduceSumKernel(float* dst, float* src, int n) {
    extern __shared__ float sharedData[];
    unsigned int tid = threadIdx.x;
    unsigned int idx = blockIdx.x * blockDim.x + threadIdx.x;
    
    sharedData[tid] = (idx < n) ? src[idx] : 0.0f;
    __syncthreads();
...
int main(int argc, char* argv[])
{
    ...
    for (int iter = 0; iter < 1000; ++iter)
    {
        ...
        case 2:
            ...
            int sharedMemSize = MAX_THREADS * sizeof(float);
            reduceSumKernel<<<numBlocks, MAX_THREADS, sharedMemSize>>>(gpuResult2, gpuResult1, MAX_BLOCKS * MAX_THREADS);
            reduceSumKernel<<<1, MAX_BLOCKS, MAX_BLOCKS * sizeof(float)>>>(gpuResult1, gpuResult2, MAX_BLOCKS);
            cudaMemcpy(&finalDotProduct, gpuResult1, sizeof(float), cudaMemcpyDeviceToHost);
            ////////////////////////////////
