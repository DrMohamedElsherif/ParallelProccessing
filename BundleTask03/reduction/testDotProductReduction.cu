 ...
 */
 ...
 /////// MY CODE
 __global__ void reduceKernel(float* dst, float* src, int numElements)
 {
     // get the global thread index, which uniquely identifies the thread
     // across all blocks and threads in the grid. 
     __shared__ float sharedData[1024];
     unsigned int threadNo= threadIdx.x;
     unsigned int globalId= blockIdx.x * blockDim.x + threadIdx.x;

     if (globalId < numElements) {
        sharedData[threadNo]= src[globalId];
     } else {
        sharedData[threadNo] = 0.0f;
     }

     __syncthreads(); 

     // Set Stride to half of the block size, and it keeps halving in each iteration.

     for (unsigned int stride = blockDim.x / 2; stride > 0; stride >>= 1)
     {
         // checks if the thread is within the current "stride" range of elements.
         // In each step, we reduce the problem by ha
         // will combine values from elements that are stride apart.
         if (threadNo < stride)
         {
             // The current thread combines its value with the value of another thread that is strde elements ahead
             sharedData[threadNo] += sharedData[threadNo + stride]; // Combine two elements
         }
     ...
     // Only the thread with index 0  writes the final reduced result to the 'dst' array.
     if (threadNo == 0)
     {

         dst[blockIdx.x] = sharedData[0]; 
     }
 }
...
int main(int argc, char* argv[])
{
    ...
    for (int iter = 0; iter < 1000; ++iter)
    {
        ...
        case 2:
            ...
            //// MY CODE
            // First redcution
            dim3 reduceBlockGrid(MAX_BLOCKS);
            dim3 reduceThreadBlock(MAX_THREADS / 2); // Here we shall use half of the threads for reduction

            reduceKernel<<<reduceBlockGrid, reduceThreadBlock>>>(gpuResult2, gpuResult1, MAX_BLOCKS * MAX_THREADS);

            // Second reduction
            dim3 secondReduceBlockGrid(1);                 // Only 1 block as in the ass. sheet
            dim3 secondReduceThreadBlock(MAX_BLOCKS);  // Use MAX_BLOCKS threads

            reduceKernel<<<secondReduceBlockGrid, secondReduceThreadBlock>>>(gpuResult2, gpuResult2, MAX_BLOCKS);

            // Copy the reduced result back to cpuResult
            cudaMemcpy(cpuResult, gpuResult2, sizeof(float), cudaMemcpyDeviceToHost);

            // The final result is now the first element in cpuResult
